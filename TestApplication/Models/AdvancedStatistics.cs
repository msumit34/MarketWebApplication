namespace TestApplication.Models
{
    public class AdvancedStatistics
    {
        ...
        public List<AdvancedStatistics> setListOfAdvancedStatisticsChartApi(string[][] values)
        {
            try
            {
                List<AdvancedStatistics> stats = new List<AdvancedStatistics>();
                double result = 0.00;
                foreach (var v in values)
                {
                    if (v.Length != 23)
                        continue;
                    stats.Add(new AdvancedStatistics()
                    {
                        ...
                        dateOfEntry = DateTime.Today.ToString()
                    }); ;
                }
